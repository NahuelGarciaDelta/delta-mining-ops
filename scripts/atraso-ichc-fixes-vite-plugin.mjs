  ...
  return {
    ...
    transform(code,id){
      ...
      if(id.endsWith('/src/modules/oficina-tecnica/OficinaTecnicaModule.jsx')){
        const atrasoSourcePattern=/const\s+rop02Prod\s*=\s*useMemo\(\(\)\s*=>\s*atrasoSource\.filter\(r\s*=>[\s\S]*?\),\s*\[atrasoSource\]\s*\);/;
        if(!atrasoSourcePattern.test(s)){
          throw new Error('[delta-atraso-ichc-fixes] No se encontro el filtro de ViewAtrasoROP02. Se cancela el build para evitar desplegar Atraso sin camiones/camionetas.');
        }
        s=s.replace(
          atrasoSourcePattern,
