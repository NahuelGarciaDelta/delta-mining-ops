...
function revalidateDatasetInBackground_(dataset,params,cached){
  ...
  fetchSyncVersions(APPS_SCRIPT_URL).then(sync=>{
    if(!sync)return null;
    if(versionsDiffer_(cached?.versions||{},sync?.versions||{}))return fetchDatasetPage(dataset,params);
    return null;
  }).catch(()=>{
    // La vista ya dispone de datos válidos; una falla de sincronización nunca debe bloquearla.
  });
}
...
export async function getDataset(dataset,params={}){
  ...
  // Las vistas críticas que pidan frescura explícita esperan la validación de versión.
  // Las vistas generales (incluida Equipos) reciben inmediatamente el último snapshot
  // completo y luego se sincronizan en segundo plano. Así todas las PCs convergen a la
  // misma fuente sin dejar la interfaz esperando indefinidamente al endpoint de versiones.
