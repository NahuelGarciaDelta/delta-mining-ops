...
function countStockCritical(){try{const raw=JSON.parse(localStorage.getItem("dm_control_stock_excel_v1")||"[]");return safeArr(raw).filter(r=>Number(r.saldo??r.Saldo??0)<Number(r.minimo??r.Mínimo??r.Minimo??0)).length}catch(_){return null;}}
...
export default function ExecutiveDashboard({rop02All=[],rop05=[],rma15=[],rawSources={},onNavigate}){
 const[remote,setRemote]=useState({pm:null,states:null,tenders:null});
 ...
 ]).then(([pm,states,tenders])=>{if(!alive)return;setRemote({pm:pm.status==="fulfilled"&&pm.value?.ok?pm.value:null,states:states.status==="fulfilled"&&states.value?.ok?states.value:null,tenders:tenders.status==="fulfilled"&&tenders.value?.ok?tenders.value:null});});return()=>{alive=false};},[]);
 const k=useMemo(()=>{
   ...
   return {active,availability,maintCost,productive,pending,stock:countStockCritical(),tenders:upcoming,criticalPm};
