...
function syncBaseTypography_(wearButton,maintenanceButton){
  ...
  wearButton.style.color=getComputedStyle(maintenanceButton).color;
}
...
function setWearVisual_(wearButton,maintenanceButton){
  ...
  if(active){
    ...
    if(maintenanceButton){
      ...
      setLabelColor_(maintenanceButton,"");
    }
  }else{
    wearButton.style.background="none";
    wearButton.style.borderLeftColor="rgba(234,179,8,.13)";
    const source=label_(maintenanceButton);
    const sourceColor=source?getComputedStyle(source).color:"";
    setLabelColor_(wearButton,sourceColor);
  }
}

function buildWearButton_(maintenanceButton){
  const button=maintenanceButton.cloneNode(true);
  button.id=WEAR_BUTTON_ID;button.type="button";button.title="Desgaste";button.removeAttribute("aria-current");
  const label=[...button.querySelectorAll("span")].find(span=>text_(span)==="Mantenimiento");
  if(label)label.textContent="Desgaste";
  button.style.background="none";button.style.borderLeftColor="rgba(234,179,8,.13)";
...
function install_(){
  const maintenanceButton=findMaintenanceButton_();if(!maintenanceButton)return;
  let wearButton=document.getElementById(WEAR_BUTTON_ID);
  if(!wearButton||wearButton.parentNode!==maintenanceButton.parentNode){wearButton?.remove();wearButton=buildWearButton_(maintenanceButton);maintenanceButton.parentNode.insertBefore(wearButton,maintenanceButton.nextSibling);}
  setWearVisual_(wearButton,maintenanceButton);
}

function handleNavClick_(event){
  const button=event.target?.closest?.("nav button");if(!button||button.id===WEAR_BUTTON_ID)return;
  if(sessionStorage.getItem(WEAR_FLAG)==="desgaste"){sessionStorage.removeItem(WEAR_FLAG);window.dispatchEvent(new CustomEvent(WEAR_CLOSE_EVENT));}
}

export function installGlobalWearSidebarBridge(){
  if(typeof window==="undefined"||typeof document==="undefined")return()=>{};
  if(window.__dmWearSidebarBridgeInstalled)return()=>{};
  window.__dmWearSidebarBridgeInstalled=true;sessionStorage.removeItem(WEAR_FLAG);install_();
  const observer=new MutationObserver(()=>install_());observer.observe(document.body,{childList:true,subtree:true});
  document.addEventListener("click",handleNavClick_,true);
  return()=>{observer.disconnect();document.removeEventListener("click",handleNavClick_,true);document.getElementById(WEAR_BUTTON_ID)?.remove();window.__dmWearSidebarBridgeInstalled=false;};
}
